...
relationships:
  tbl.txn_ref_dt:
    - "tbl2.txn_value_dt" #can define relationship with a column in another table inside dataset, pattern -> ?(<dataset uuid>.)<table_name>.<column_name>?(.<nested_column_name>)
    - "53581432-6c55-4ba2-a65f-72344a91553a.txn_file.transaction_date"  #or define relationship with column in another dataset by referring using the dataset uuid
